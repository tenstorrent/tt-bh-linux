...
public:
    ...
    void process_queue_start(int queue_idx, uint8_t* addr, uint64_t len) override {
        // For network, set up header fields if needed
        if (queue_idx==0){
          ...
          //  struct virtio_net_hdr_mrg_rxbuf* req = reinterpret_cast<struct virtio_net_hdr_mrg_rxbuf*>(addr);
    ...
    void process_queue_complete(int queue_idx, uint8_t* addr, uint64_t len) override {
        // For network, nothing special needed
    }
