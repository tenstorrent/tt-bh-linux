...
public:
    ...
    VirtioBlk(int l2cpu_idx, std::atomic<bool>& exit_flag, std::mutex& interrupt_register_lock)
        : VirtioDevice(l2cpu_idx, exit_flag, interrupt_register_lock, 33, (2ULL * 1024 * 1024)) {
        
        num_queues = 1;
        device_features_list[0] = 0;
        device_features_list[1] = 1<<(VIRTIO_F_VERSION_1-32);
        
        fd = open("debian-riscv64.img", O_RDWR);
        if (fd == -1) {
    ...
    void process_queue_data(int queue_idx, uint8_t* addr, uint64_t len) override {
        ...
        case VIRTIO_BLK_T_OUT:
            ...
            break;
        default:
            printf("Unimplemented Request Type Flags: %d Len: %lu\n", req->type, len);
        }
    }
