...
public:
    ...
    VirtioBlk(int l2cpu_idx, std::atomic<bool>& exit_flag, std::mutex& interrupt_register_lock, const std::string& image_path, int interrupt_num = 33, uint64_t mmio_offset = 6ULL * 1024 * 1024)
        : VirtioDevice(l2cpu_idx, exit_flag, interrupt_register_lock, interrupt_num, mmio_offset), disk_image_path(image_path) {
        ...
        mapped_data = (uint8_t*)mmap(NULL, file_size, PROT_READ|PROT_WRITE, MAP_SHARED|MAP_PRIVATE, fd, 0);
